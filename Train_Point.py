...
class Mainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        for step,(images, heatmap_gt, heatmap_posweight, heatmap_offset, heatmap_off_mask, joints, num_objs) in enumerate(self.Data_dict['Train_loader']):
            ...
            push_loss, pull_loss = self.Loss_meter['tag_loss'](out_dicts['tag'], joints)
            tag_loss  = push_loss.sum() + pull_loss.sum()
    ...
    def val_epoch(self, epoch):
        ...
        log_loss = 0
        with torch.no_grad():
            for step,(images, heatmap_gt, heatmap_posweight, heatmap_offset, heatmap_off_mask, joints, num_objs) in enumerate(self.Data_dict['Val_loader']):
                batch_objs = sum(num_objs)
                if batch_objs == 0:
                    batch_objs = 1
