...
class LDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if(randrf(0,1) < 0.5):
            # image, info_points = augmentWithFlip(image, info_points)
        ...
        box_wh            = np.zeros((2, self.fm_height, self.fm_width), np.float32)
        ...
        for jo_id, group in enumerate(info_points):
            ...
            for p_id, point in enumerate(group):
                ...
                point[2] = 2 if point[2]==0 else point[2]
                point[3] = 2 if point[3]==0 else point[3]
                box_wh[0, int_cy, int_cx] = re_scale*point[2]/self.stride
                box_wh[1, int_cy, int_cx] = re_scale*point[3]/self.stride

        return torch.tensor(newImage), heatmap_gt, heatmap_posweight, heatmap_offset, heatmap_off_mask, box_wh, joint_num
