...
class SimBottleneck(nn.Module):
    ...
    def __init__(self, cin, cout, shortcut=True, g=1, expansion=0.5,
        ...
        self.conv1 = ConvBase(cin, c_, 1, 1, norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.conv2 = ConvBase(c_, cout, 3, 1, groups=g, norm_cfg=norm_cfg, act_cfg=act_cfg)
...
class SimBottleneckCSP(nn.Module):
    ...
    def __init__(self, cin, cout, n=1, shortcut=True,
        ...
        self.m = nn.Sequential(*[SimBottleneck(c_, c_, shortcut, g, expansion=1.0) for _ in range(n)])
...
class BasicRFBBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.ConvLinear = ConvBase(6*inter_planes, out_planes, kernel_size=1, stride=1, act_cfg=None)
        self.shortcut = ConvBase(in_planes, out_planes, kernel_size=1, stride=stride, act_cfg=None)
